...
void gameMenu(struct roomData *gameRooms[]) {
	...
	//Loops through all of the connections in this
		...
		while (entryMatch == 0) {		//#2
			...
			for (i = 0; i < gameRooms[curRoom]->numConns; i++) {
			...
			for (i = 0; i < gameRooms[curRoom]->numConns; i++) {//#3
				if (strcmp(usrInput, gameRooms[curRoom]->conn[i]) == 0) {
					...
					for (i = 0; i < 7; i++) {//#4 Loop
						...
						movCnt++;
						ptv[movCnt] = i;
						printf("in the if statement to find the next room index.\n");
						printf("the move count is %d.\n", movCnt);
						}
					}//#4 Loop End
				}
			}//#3
	...
	printf("\n");
	printf("YOU HAVE FOUDN THE END ROOM. CONGRATULATIONS!.\n");
...
//	int i; 						//Generic counter
	...
	printRoomData(gameRooms);		//Just for testing.  To be removed.
